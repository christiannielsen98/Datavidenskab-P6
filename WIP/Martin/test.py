...
def replace_short_remove_fill(string):
    fill_words = ['Whether', 'Warm', 'Used', 'Usage', 'Total', 'Supply', 'Subset', 'Status', 'Simulate', 'Sens',
                  'Power', 'Plugged', 'Peninsula', 'Opening', 'Number', 'Note', 'Needed', 'Midnight', 'Located', 'Load',
                  'load', 'Leg', 'Items', 'Into', 'Instrumentation', 'Instantaneous', 'Inst', 'Indicate', 'Includes', 'Get',
                  'From', 'Emulator', 'Electrical', 'Consumption', 'Appliance', 'Adding', 'Activated', '(1:Yes,O:No)', ';', '.', ',']
    something_dict = {
...
for hourly in [True, False]:  # True, False
    for year in [1, 2]:  # 1, 2
        ...
        for status_orig, status_row in status_attributes.iterrows():
            ...
            for sub_system in ['Load', 'Elec']:  # meta.loc[(lambda self: self['Units'] == 'W'), 'Subsystem'].unique():
                included = False
                for location_condition in location_conditions:
                    ...
                    while not included and condition_index < len(conditions):
                        matches_dict[sub_system][status_orig] = direct_matcher(sub_system,
        ...
        for key, value in matches_dict.items():
            ...
            missing_statuses = [status for status, not_found in value.items() if not_found is None]
            print(missing_statuses)
            print(len(value.keys()) - i)
